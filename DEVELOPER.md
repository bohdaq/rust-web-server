...
Mechanically, this is the first place a response needs to be routed back to a specific in-flight call rather than just broadcast or session-targeted like every prior notification. `McpServer` mints a fresh request id, registers an `mpsc::Sender<Result<String, String>>` for it in `pending_replies: Arc<Mutex<HashMap<String, mpsc::Sender<...>>>>`, sends the request to the caller's session via the existing `send_sse_to_sessions` (from `resources/subscribe`), and blocks on the matching receiver. The client's eventual `POST /mcp` reply is a JSON-RPC *response* — no `method` field, by definition — so `handle_request_with_context`/`handle_batch` now check a method-less body with a recognized `id` against `pending_replies` (via `try_deliver_sampling_response`) before falling back to the old "Missing method" `INVALID_REQUEST` error; a matching id delivers the response through the channel and unblocks the waiting call, a non-matching one still gets rejected as invalid, exactly as before this existed. This mechanism (`McpContext::send_and_wait`) is shared with `roots/list` below — `sample()` and `list_roots()` are both thin wrappers around it, differing only in the method name/params they send and how they parse the `result`.

`ctx.sample()` fails fast, before sending anything, if: the client's `initialize` call never declared `params.capabilities.sampling` (checked via a `StoredClientInfo.supports_sampling` bool, since sampling is a *client* capability the spec has the client declare, not something the server advertises); there's no session id to address the request to; or `ctx` has no live server behind it. It fails with a timeout error if the client never responds — including if it has no `GET /mcp` SSE connection open for that session at all, since there's no separate "not connected" signal from `send_sse_to_sessions`.
...
**`roots/list` and `notifications/roots/list_changed`:** a client can advertise which filesystem roots (workspace directories, mounted volumes) it has access to, so a file-system-aware tool knows to stay within them. `ctx.list_roots(timeout)` reuses the exact `send_and_wait` mechanism `sample()` is built on — a server-initiated `roots/list` request over SSE, answered by the client's `POST /mcp`:
...
Unlike `sample()`, the result is **cached per session** in a new `StoredClientInfo.roots: Option<Vec<McpRoot>>` field (`None` means "never fetched, or invalidated") — a tool handler can call `list_roots()` on every invocation without worrying about spamming the client with redundant `roots/list` round trips; only the first call in a session actually sends anything. `notifications/roots/list_changed` (special-cased in both dispatch paths, ahead of the generic notification-swallowing branch, the same way `notifications/cancelled` already was) clears that session's cached entry back to `None` via `invalidate_roots_cache`, correlated purely by the notification's own `Mcp-Session-Id` header rather than any params (the spec's `list_changed` notification carries none — it's "your cache for *this connection* is stale," nothing more specific). The next `list_roots()` call in that session does a fresh round trip.

Fails fast under the same conditions as `sample()` (translated to roots): the client never declared `params.capabilities.roots`, no session id, or no live server behind `ctx`; otherwise times out if the client never answers a live request.

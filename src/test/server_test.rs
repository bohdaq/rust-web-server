...
#[test]
fn check_range_response_for_not_proper_range_header_range_start_bigger_than_filesize() {
    let uri = "/static/test.txt";
    let url = Server::get_static_filepath(uri);
    ...
    let content_range = response.content_range_list.get(0).unwrap();
    assert_eq!(content_range.body, Range::ERROR_START_IS_BIGGER_THAN_FILESIZE_CONTENT_RANGE.as_bytes());
}
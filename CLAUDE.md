...
`.require_oauth(provider: sso::OidcProvider, audience)` (MCP 2025-03-26 OAuth 2.0 authorization, `sso` feature) is an alternative to `.require_bearer(token)`: instead of one static shared secret, it verifies a client's `Authorization: Bearer <jwt>` against a live JWKS endpoint by reusing `sso::jwks::JwksCache` directly — the "leverage point" this feature's own backlog entry named, not reimplemented. Stored as `oauth: Option<Arc<OAuthConfig>>` (`OAuthConfig { jwks, provider, audience }`), gated `#[cfg(feature = "sso")]` end to end since `mcp` itself carries no Cargo feature of its own. `execute()` checks `self.oauth` before `self.auth_token` — if both are configured, OAuth wins and the static token is never checked, an explicit tie-break for an unsupported-in-practice combination. On success, the verified `sso::OidcClaims` (JSON-serialized) populates `McpContext.auth_claims`; on any failure (missing/malformed/invalid-signature/wrong-audience/wrong-issuer/expired token) the request gets the same `401` + `WWW-Authenticate: Bearer` `unauthorized()` response `.require_bearer()` already used. `GET /.well-known/oauth-authorization-server` is served automatically whenever OAuth is configured (checked ahead of the `/mcp` path match, since it's a sibling top-level path) with a minimal, not-full-RFC-8414 document (`issuer`/`authorization_endpoint`/`token_endpoint`/`jwks_uri` sourced straight from `provider`) — falls through to the fallback app when OAuth isn't configured. `require_oauth` takes an `OidcProvider` (a preset or the result of `OidcProvider::discover(issuer)`) rather than a bare JWKS URL string, correcting an ambiguity in the backlog entry's own sketch (which named its parameter `jwks_url` but gave a `.well-known/openid-configuration` discovery URL as the example value — two different things).
...
### SSO (OAuth 2.0 / OIDC client, `sso` feature; Authorization Server, `sso-server` feature; SAML 2.0 SP, `sso-saml` feature)
...
`saml::SamlSp` (`sso-saml` feature, implies `sso`) — SAML 2.0 Service Provider, the SAML counterpart to `OidcAuth`. A `Middleware` intercepting `GET /saml/metadata`, `GET /saml/login` (renders an auto-submitting HTML form POSTing an unsigned `AuthnRequest` — HTTP-POST binding, not HTTP-Redirect, since this crate has no DEFLATE dependency and requests are never signed since this crate has no private-key PEM parser, the same reason `AuthServer` uses HS256), `POST /saml/acs`, `GET /saml/logout` (local session destruction only, no IdP `LogoutRequest`, mirroring `OidcAuth`'s own logout). Four submodules: `saml/xml.rs` — a small hand-rolled XML parser (not `quick-xml`, deliberately: this crate hand-rolls every text format it parses; the parser rejects any `<!DOCTYPE>` outright, eliminating XXE by never implementing entity resolution) tracking each `XmlNode`'s exact byte range in the source document; `saml/der.rs` — a minimal DER/ASN.1 TLV walker that locates an X.509 certificate's `subjectPublicKeyInfo` field (skipping `tbsCertificate`'s preceding fields by length) and hands the slice to `rsa::pkcs8::DecodePublicKey` — not a general X.509 parser; `saml/metadata.rs` — `SamlIdpMetadata::from_file`/`::from_url`/`::from_str`, preferring an `HTTP-POST`-binding `SingleSignOnService` entry; `saml/assertion.rs` — signature verification and claim extraction.

**Signature verification is byte-exact against the transmitted document, not full XML C14N** — `parse_and_verify` computes the SHA-256 digest over the `Assertion`'s own raw byte range with the `Signature` element's raw byte range excised (the enveloped-signature transform, applied to literal bytes rather than a canonicalized form), and verifies `SignatureValue` (RSA-SHA256 only) over `SignedInfo`'s raw byte range. This is a deliberate scope decision (documented in `assertion.rs`'s module docs), not an oversight: it fails closed, not open — an IdP that reformats XML between signing and transmission would have legitimate logins rejected, never would a forged assertion be accepted. **XSW (XML Signature Wrapping) mitigation is structural**: `parse_and_verify` requires exactly one `Assertion` in the response (rejecting zero or more than one), requires `Signature` to be a direct child of that same `Assertion`, requires `Reference/@URI` to point at that assertion's own `ID`, and reads every claim from that same parsed node — never a second search pass an attacker could point elsewhere. `EncryptedAssertion` is rejected outright (no XML decryption implemented).

`saml::AttributeMap` (`.new().map(saml_attribute_name, field)`) translates raw SAML attribute names into `saml::SamlClaims { name_id: String, attributes: HashMap<String, String> }` — **not** `jwks::OidcClaims`, since `OidcClaims` was deliberately built without a `groups` field (see above) and this feature's own design sketch maps exactly a `groups` attribute; reusing `OidcClaims` would silently drop it. `SamlSp::claims(request)`/`::name_id()`/`::attr(request, field)` read the `X-Rws-Saml-Claims` header the same way `OidcAuth`'s equivalents read `X-Rws-Oidc-Claims`.

`sso-saml` adds no new Cargo dependency beyond what `sso` already pulls in — the spec's own suggestion of a `quick-xml` dependency was not taken (see above).

...
| `JwtLayer` | `auth` | HS256 Bearer JWT validation middleware; `::rs256`/`::es256` verify RS256/ES256 against a static public key (`auth-asymmetric` feature) |
...
| `AzureBlobStorage` | `storage` | Azure Blob Storage via Shared Key HMAC signing or Managed Identity, no Azure SDK (`storage-azure` feature) |

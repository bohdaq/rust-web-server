...
:::caution[Coming Soon: TLS to upstream]
All upstream connections are plain TCP. HTTPS backends are not yet supported. For encrypted upstreams, use `H2ReverseProxy` with an H2 cleartext backend or terminate TLS at the backend itself.
:::

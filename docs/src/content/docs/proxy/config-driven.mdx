...
Backends must be `"host:port"` strings. Do not include `http://` — scheme prefixes are stripped automatically. TLS to upstreams is not yet supported.
:::

:::caution[Coming Soon: upstream TLS]
HTTPS backends (`https://`) are parsed but connections are made over plain TCP. Mutual TLS to upstreams is not yet implemented.
:::
